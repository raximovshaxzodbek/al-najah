...
export default function TaskOneQuestion3() {
  ...
  const handleRecordingComplete = async (audioData) => {
    ...
    const fileName = `1_${partOneData[2]?.question}.webm`; // Use the actual file name if available in audioData
    ...
    if (oneAudio && twoAudio) {
      if (warningSecond > 0) {
        ...
        if (second > 0) {
          ...
          navigate("/task_id=1/question=4");
        }
      }
    }
  }, [warningSecond, second, oneAudio, twoAudio]);

  const playlist = [taskQuestionAudio, URL + partOneData[2]?.audio];
    ...
    <>
      ...
      >
        ...
        <div className="flex w-full flex-col gap-[20px] py-4">
          <h1 className="arabic-text text-center text-2xl font-normal text-[#118FCE]">
            أنت في الجزء الأول
          </h1>
          <div className="flex flex-col items-center gap-3">
            {oneAudio ? (
              <h2 className="arabic-text text-xl font-normal md:text-4xl">
                <span className="number">٣</span> {partOneData[2]?.question}
              </h2>
            ) : null}
          </div>
        </div>
