...
import taskQuestionAudio from "../../../assets/audio/task1/question4.aac";

export default function TaskThreeQuestion4() {
  ...
  const handleRecordingComplete = async (audioData) => {
    ...
    const fileName = `3_${partThreeData.question4}.webm`;
    ...
    if (oneAudio && twoAudio) {
      if (warningSecond > 0) {
        ...
        if (second > 0) {
          ...
          navigate("/task_id=3/question=5");
        }
      }
    }
  }, [warningSecond, second, oneAudio, twoAudio]);

  const playlist = [taskQuestionAudio, URL + partThreeData.audio4];
          ...
          <div className="flex flex-col items-center gap-3">
            {oneAudio ? (
              <h2 className="arabic-text text-xl font-normal md:text-4xl">
                <span className="number">٤</span> {partThreeData.question4}
              </h2>
            ) : null}
          </div>
        </div>
